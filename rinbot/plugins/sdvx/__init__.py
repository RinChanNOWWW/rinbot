...
async def recent_play(bot: Bot, event: Event):
    ...
    try:
        ...
        await bot.send(
            message="您还未绑定卡号。使用 `\\bind CARD` 绑定卡号。", event=event
        )
        return
    except NoPlays:
...
async def today_play(bot: Bot, event: Event):
    ...
    try:
        ...
        start_time = time.strftime("%H:%M:%S", time.localtime(start["timestamp"]))
...
bye = on_keyword(
    set(["下线", "拜拜", "睡觉", "bye", "Bye"]), rule=to_me(), permission=SUPERUSER
)
